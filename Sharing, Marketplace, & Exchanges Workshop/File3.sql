...
select count (distinct postal_code) from history_day where country = 'US' and ( postal_code like '481%' or postal_code like '482%' ) limit 100;

create or replace view demo_db.public.DETROIT_ZIPS  as 
select distinct postal_code from weathersource.standard_tile.history_day where country = 'US' and ( postal_code like '481%' or postal_code like '482%' );

select count(1) from demo_db.public.DETROIT_ZIPS; 
select count(1) from weathersource.standard_tile.history_day;

select count(*) 
from weathersource.standard_tile.history_day hd join 
demo_db.public.detroit_zips dz on hd.postal_code = dz.postal_code;

-- Date ranges 
select 
    min(date_valid_std)  as min_Date, 
    max(date_valid_std) as max_date,
    'history_day' as table_name
from history_day
...
select 
    min(date_valid_std)  as min_Date, 
    max(date_valid_std) as max_date,
    'forecast_day' as table_name
from forecast_day;


select 
    date_valid_std, round( avg(avg_cloud_cover_tot_pct) ,2) as cloud_cover_pct
from forecast_day fd join demo_db.public.detroit_zips dz on fd.postal_code = dz.postal_code
    group by 1
    order by 1 ;
